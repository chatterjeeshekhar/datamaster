...
error:function (){}
  ...
  function recoverFunction(str) {
  if (confirm('Restore this record?') == true) {
 $("#divLoading").show();
jQuery.ajax({
url: "add-item.php",
data: "ac=0&q="+str,
...
$userac = $_SESSION['login_user'];
  ...
  $mysqlstat = "SELECT * FROM emails ORDER BY ID DESC";
...
//echo "<td>" . $cntbody . "</td>";
$asuser1 = "'".$asuser."'";
echo "<td><button type='button' class='btn btn-primary' onclick='getFunctions($emid)' data-toggle='modal' data-target='#myModal'>Click to view ($emid)</button></td>";
  echo "<td>";
  ...
  {
  echo "<a onclick='recoverFunction($emid)'><img src='files/tick.png'></a>";
  }
  echo "</td>";
  echo "</tr>";
  }
