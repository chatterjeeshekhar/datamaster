...
function removeFunction(str) {
   if (confirm('Are you sure you want to delete this group (All contacts in group will be deleted) ?') == true) {
 $("#loaderIcon").show();
jQuery.ajax({
url: "deletecontact.php",
data:'q=group&str='+str,
type: "POST",
            ...
            <div class="col-xs-12">
              ...
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">
 <button type='button' class='btn btn-primary' data-toggle='modal' data-target='#myModal'>Add New</button> <button type='button' class='btn btn-success' data-toggle='modal' data-target='#myModal3'>Import from Excel</button>
</h3>
                </div><!-- /.box-header -->
                <div class="box-body">
...
$mysqlstat = "SELECT * FROM groups where user='$userac' and level=1 ORDER BY grpname ASC";
...
if($rows > 0)
{
echo "<th>ID</th><th>Group Name</th><th>List Linked Contacts</th><th>Delete</th></tr></thead><tbody>";
...
echo "<td>" . "<button type='button' class='btn btn-primary' onclick='getFunction2($emid)' data-toggle='modal' data-target='#myModal2' href='#myModal2'>Click to view</button>" . "</td>";
...
?>
  ...
  <form method="post" action="" onsubmit="return ValidateForm()" autocomplete="off">                  <div class="box-body">
                  ...
                  <div class="box-footer">
                    <button type="submit" name="submit" value="submit" id="Submit" class="btn btn-primary"><i class="fa fa-send-o"></i>  Add new group</button>
                  </div>
                </form> 
        </form>
        <?php
...
$sql = "INSERT INTO groups (grpname, user, time, level) VALUES ('".$varname."', '".$usern."', (NOW() + INTERVAL 630 MINUTE), '1');";
...
echo '</script>';
  ...
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">List of contacts associated with this group</h4>
      </div>
      <div class="modal-body" id="rslt2">

</div>
        <script>

  function getFunction2(str) {
   
  if (str.length == 0) { 
          document.getElementById("rslt2").innerHTML = "Error";
          return;
      } else {
          document.getElementById("rslt2").innerHTML="<?php 
$phpvar='"+str+"';
$ppwp = $phpvar;
$urlstr = $_SESSION['projectweb']."admin/getgrp.php?i=".$ppwp;
...
<div class="modal fade bs-example-modal-lg" id="myModal3" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
  <div class="modal-dialog modal-lg" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
        <h4 class="modal-title" id="myModalLabel">Add Contacts from Excel Sheet</h4>
      </div>
      <div class="modal-body" id="rslt">
