...
$query = mysqli_query($con, "select * from login where password=md5('$password') AND user='$username'");
...
if($chkrows>0){
$query11 = mysqli_query($con, "update login set active='N' where user='$username'");
...
mysqli_query($con, "update login set lastlogin=(NOW() + INTERVAL '$tz' MINUTE) where user='$username'");
mysqli_query($con, "insert into loginlog (user, ip, timestamp) VALUES ('$username','$ipadd',(NOW() + INTERVAL '$tz' MINUTE))");
mysqli_query($con, "insert into banip (user,IP,active,timestamp) values ('$username','$ipadd','1',(NOW() + INTERVAL '$tz' MINUTE))");
 exit(header("Location: home.php"));
}
else
if($row2['active'] == "A") {
$error = "Account pending activation: <b><a href='/confirm.php'>Activate now</a></b>";
}
else 
{
